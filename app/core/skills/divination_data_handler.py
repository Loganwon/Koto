...
class DivinationDataHandler:
    ...
    def _extract_entities(self, question: str, domain: str) -> Dict[str, Any]:
        ...
        if domain == "sports_esports":
            ...
            # 提取城市名
            cities = re.findall(
                r"(北京|上海|广州|深圳|杭州|成都|西安|南京|天津|重庆|苏州|杭州)",
                question,
            )
            if cities:
    ...
    def _build_metadata(
        ...
        metadata["series_target_wins"] = self._infer_series_target_wins(
            question, local_data
        )
        metadata["score_requested"] = any(
            token in question for token in ["比分", "几比几", "多少比多少", "score"]
        )
        metadata["winner_requested"] = any(
            token in question for token in ["谁赢", "谁会赢", "谁胜", "赢"]
        )
        return metadata

    def _infer_series_target_wins(
        self, question: str, local_data: Dict[str, Any]
    ) -> int:
    ...
    def _fetch_local_data(
        self, domain: str, entities: Dict[str, Any]
    ) -> Tuple[Dict, float]:
        """从本地数据源获取相关数据"""
        ...
        if domain == "sports_esports" and "team1" in entities and "team2" in entities:
            ...
            if team1_data and team2_data:
                retrieved_data = {
                    "team1": team1_data,
                    "team2": team2_data,
                    "recent_encounters": self._find_recent_matches(
                        team1, team2, data_source
                    ),
                }
                confidence = 0.75
    ...
    def _find_recent_matches(
        self, team1: str, team2: str, data_source: Dict
    ) -> List[Dict]:
        """查找两支队伍最近的对战记录"""
        ...
        for match in matches[-20:]:  # 查看最近20场
            if match.get("team1") in [team1, team2] and match.get("team2") in [
                team1,
                team2,
            ]:
    ...
    def generate_data_driven_prediction(
        self, context: DivinationContext, tarot_cards: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        基于数据和塔罗牌生成预测
    ...
    def _predict_sports_match(
        ...
        team1_tendency = (
            (team1_strength / total_strength) if total_strength > 0 else 0.5
        )
        ...
        winner_probability = (
            final_tendency if winner == team1_name else (1 - final_tendency)
        )
        winner_probability_pct = int(round(winner_probability * 100))
        series_target_wins = context.metadata.get("series_target_wins", 2)
        predicted_score = self._predict_series_score(
            series_target_wins, winner_probability
        )
        scoreline = self._format_scoreline(
            winner, team1_name, team2_name, predicted_score
        )

        data_factors = []
        if team1_strength > team2_strength:
            data_factors.append(
                f"{team1_name} 最近状态更佳（实力评分 {team1_strength:.0f}）"
            )
        else:
            data_factors.append(
                f"{team2_name} 最近状态更佳（实力评分 {team2_strength:.0f}）"
            )

        if historical_rate is not None:
            data_factors.append(
                f"历史对战中 {team1_name} 胜率为 {historical_rate*100:.0f}%"
            )
    ...
    def _calculate_historical_winrate(
        self, context: DivinationContext
    ) -> Optional[float]:
        """计算两队之间的历史胜率"""
    ...
    def _get_tarot_adjustment(self, tarot_cards: List[Dict[str, Any]]) -> float:
        ...
        positive_cards = sum(
            1 for card in tarot_cards if not card.get("reversed", False)
        )
    ...
    def _predict_series_score(
        self, series_target_wins: int, winner_probability: float
    ) -> Tuple[int, int]:
    ...
    def format_prediction_for_prompt(self, prediction: Dict[str, Any]) -> str:
        """将预测结果格式化为提示词友好的字符串"""
        lines = []
        ...
        if prediction.get("winner_probability") is not None:
            lines.append(
                f"  胜者赢面：{int(round(prediction['winner_probability'] * 100))}%"
            )
