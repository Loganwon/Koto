...
class _WordGenerator:
    ...
    def generate(self, req: DocGenRequest, output_path: Path) -> None:
        ...
        for s in req.sections:
            ct = s.content_type.lower() if isinstance(s.content_type, str) else s.content_type.value
            ...
            if ct == ContentType.HEADING:
                ...
                for row_idx, row_data in enumerate(rows_data):
                    ...
                    for col_idx, cell_val in enumerate(row_data):
                        ...
                        run = cell.paragraphs[0].runs[0] if cell.paragraphs[0].runs else cell.paragraphs[0].add_run(str(cell_val))
                        run.font.size = Pt(10)
                        run.font.name = "微软雅黑"
                        try:
                            run._r.rPr.rFonts.set(qn("w:eastAsia"), "微软雅黑")
                ...
                style = "List Bullet" if ct == ContentType.BULLET_LIST else "List Number"
                for item in items:
...
class _PdfGenerator:
    ...
    def _register_fonts(self) -> Tuple[str, str]:
        ...
        logger.warning("[DocGen] 未找到中文字体，PDF 正文可能显示为方框（推荐安装微软雅黑）")
    ...
    def generate(self, req: DocGenRequest, output_path: Path) -> None:
        ...
        for s in req.sections:
            ...
            if ct == ContentType.HEADING:
                ...
                story.append(
                    ListFlowable(
                        [ListItem(Paragraph(str(item), style_body), bulletColor=primary_color) for item in items],
                        bulletType="bullet",
                    )
                )
...
class _ExcelGenerator:
    ...
    def _apply_header_style(self, ws, row_idx: int, col_count: int, preset: Dict, bg_rgb, fg_rgb) -> None:
    ...
    def generate(self, req: DocGenRequest, output_path: Path) -> None:
        ...
        if not sheets_to_gen:
            ...
            for s in req.sections:
                ct = s.content_type.lower() if isinstance(s.content_type, str) else s.content_type.value
                if ct == ContentType.TABLE and isinstance(s.content, list) and len(s.content) > 0:
                    if not default_headers:
                        default_headers = [str(c) for c in s.content[0]]
                        default_rows = [list(row) for row in s.content[1:]]
            if default_headers:
                sheets_to_gen = [ExcelSheet(
                    name="数据", title=req.title,
                    column_headers=default_headers, rows=default_rows,
                )]
            else:
                # 如果完全没有表格数据，生成一个说明 Sheet
                sheets_to_gen = [ExcelSheet(
                    name="说明", title=req.title,
                    column_headers=["内容"],
                    rows=[[s.content] for s in req.sections if isinstance(s.content, str)],
                )]
        ...
        for sheet_def in sheets_to_gen:
            ...
            if sheet_def.title:
                ...
                row_cursor += 1
                # 生成日期行
                ws.cell(row=row_cursor, column=1, value=f"生成时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}")
                ws.cell(row=row_cursor, column=1).font = Font(name="微软雅黑", size=9, color="888888")
                row_cursor += 1
            ...
            if headers:
                ...
                self._apply_header_style(ws, row_cursor, len(headers), preset,
                                         (hbr, hbg, hbb), (hfr, hfg, hfb))
                row_cursor += 1
            ...
            for data_row in sheet_def.rows:
                for col_idx, cell_val in enumerate(data_row, start=1):
                    ...
                    # 斑马条纹
                    if row_cursor % 2 == 0:
                        c.fill = PatternFill(start_color=stripe_hex, end_color=stripe_hex, fill_type="solid")
                    # 尝试累加数值列
                    if sheet_def.add_totals_row:
                        try:
                            totals[col_idx] = totals.get(col_idx, 0) + float(str(cell_val).replace(",", "").replace("¥", "").replace("%", ""))
                        except ValueError:
            ...
            if sheet_def.add_totals_row and totals:
                ...
                ws.cell(row=row_cursor, column=1).font = Font(name="微软雅黑", bold=True, size=10)
                for col_idx, val in totals.items():
                    if col_idx == 1:
                        continue
                    c = ws.cell(row=row_cursor, column=col_idx, value=val if isinstance(val, str) else round(val, 2))
                    c.font = Font(name="微软雅黑", bold=True, size=10, color=primary_hex)
                    c.fill = PatternFill(start_color=stripe_hex, end_color=stripe_hex, fill_type="solid")
                row_cursor += 1
            ...
            for col_idx in range(1, (len(headers) or 1) + 1):
                ...
                if sheet_def.column_widths and col_idx - 1 < len(sheet_def.column_widths):
                    ws.column_dimensions[col_letter].width = sheet_def.column_widths[col_idx - 1]
                else:
                    max_len = 0
                    for row_cells in ws.iter_rows(min_col=col_idx, max_col=col_idx):
                        for c in row_cells:
                            if c.value:
                                # 中文字符计2个单位
                                val_str = str(c.value)
                                char_width = sum(2 if ord(ch) > 127 else 1 for ch in val_str)
...
class _PptxGenerator:
    ...
    def generate(self, req: DocGenRequest, output_path: Path) -> None:
        ...
        def _set_font(run_or_para, font_name="微软雅黑", size_pt=None, bold=False, color=None):
            try:
                tf = run_or_para.font
                tf.name = font_name
                if size_pt:
        ...
        subtitle_ph = title_slide.placeholders[1] if len(title_slide.placeholders) > 1 else None

        title_ph.text = req.title
        for para in title_ph.text_frame.paragraphs:
            for run in para.runs:
                _set_font(run, size_pt=preset["title_font_size"], bold=True, color=(pr, pg, pb))
        ...
        def _new_content_slide(heading_text: str):
            ...
            if title_sp:
                ...
                for para in title_sp.text_frame.paragraphs:
                    for run in para.runs:
                        _set_font(run, size_pt=preset["h1_font_size"], bold=True, color=(pr, pg, pb))
            # Content placeholder
            content_sp = None
        ...
        def _add_text_to_tf(text: str, bold=False, size_pt=None, color=None, bullet=False):
            if current_tf is None:
                return
            para = current_tf.add_paragraph()
            run = para.add_run()
            run.text = text
            _set_font(run, size_pt=size_pt or preset["body_font_size"], bold=bold, color=color)
            if bullet:
                para.level = 1
        ...
        for s in req.sections:
            ...
            if ct == ContentType.HEADING and s.level == 1:
                ...
                _add_text_to_tf(str(s.content), bold=True, size_pt=preset["h2_font_size"], color=(sr, sg, sb))
                ...
                items = s.content if isinstance(s.content, list) else [str(s.content)]
                for item in items:
                    _add_text_to_tf(f"• {item}" if ct == ContentType.BULLET_LIST else item,
                ...
                height = min(Inches(0.35 * row_count + 0.3), prs.slide_height - Inches(2.0))

                tbl_shape = table_slide.shapes.add_table(row_count, col_count, left, top, width, height)
                tbl = tbl_shape.table
                for row_idx, row_data in enumerate(rows_data):
                    for col_idx, cell_val in enumerate(row_data):
                        ...
                        for para in cell.text_frame.paragraphs:
                            for run in para.runs:
                                is_header = row_idx == 0
                                _set_font(run, size_pt=9, bold=is_header,
