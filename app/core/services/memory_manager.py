...
class PersonalityMatrix:
    ...
    def apply_extraction(self, extracted: Dict) -> bool:
        """Apply validated single-pass personality output and persist changes."""
        if not isinstance(extracted, dict):
            return False

        changed = False
        for key, value in extracted.get("cognitive", {}).items():
            if key not in self._DEFAULT_COGNITIVE:
                continue
            if isinstance(value, (int, float)):
                old = self.data["cognitive"].get(key, 0.5)
                self.data["cognitive"][key] = round(
                    0.85 * old + 0.15 * max(0.0, min(1.0, float(value))),
                    4,
                )
                changed = True

        for topic, score in extracted.get("expertise", {}).items():
            if topic and isinstance(score, (int, float)):
                old = self.data["expertise"].get(topic, 0.0)
                self.data["expertise"][topic] = round(
                    0.80 * old + 0.20 * max(0.0, min(1.0, float(score))),
                    4,
                )
                changed = True
        if len(self.data["expertise"]) > 30:
            sorted_expertise = sorted(
                self.data["expertise"].items(), key=lambda item: item[1]
            )
            self.data["expertise"] = dict(sorted_expertise[5:])

        for goal in extracted.get("goals", []):
            if goal and goal not in self.data["goals"]:
                self.data["goals"].append(goal)
                changed = True
        self.data["goals"] = self.data["goals"][-10:]

        for theme in extracted.get("recent_themes", []):
            if not theme:
                continue
            if theme in self.data["recent_themes"]:
                self.data["recent_themes"].remove(theme)
            self.data["recent_themes"].append(theme)
            changed = True
        self.data["recent_themes"] = self.data["recent_themes"][-20:]
...
class EnhancedMemoryManager:
    ...
    def add_memory(
        ...
        try:
            ...
            logger.debug("Non-fatal", exc_info=True)  # 向量索引失败不影响主路径
    ...
    def auto_extract_from_conversation(
        ...
        *,
        ...
        """Extract profile, memories and graph triples with one LLM call."""
        from app.core.memory.conversation_memory_extractor import (
            ConversationMemoryExtractor,
        )

        return ConversationMemoryExtractor.extract_and_apply(
            self,
            user_msg,
            ai_msg,
            history,
    ...
    def search_vector_memories(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            logger.debug("Non-fatal", exc_info=True)  # FAISS 失败 → 降级
