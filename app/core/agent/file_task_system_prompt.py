...
def build_file_task_system_prompt(
    *,
    ...
    return (
        ...
        "5. 当用户要求创建 DOCX/Word 但没有明确目标路径时，在源文件同目录创建清晰命名的输出文件，例如“源文件名_分步总结.docx”；不要因为缺少目标路径而只输出文字。用户明确提到某个输入目录/文件夹并给出相对输出文件名时，默认把输出文件写入该输入目录，不要写到 workspace 根目录。\n"
        ...
        "19. 对文件总结、解读和只读分析，所有事实、人物、数据、章节结论和案例都必须能在已读取的文件上下文或工具结果中找到依据；不得补造页码、引文、统计、案例或作者立场。必须区分“原文事实”和“你的概括/推断”，推断要明确标为概括或解读。只有用户明确要求逐字引文、且已读取内容包含完全一致的文字时，才可以使用引号或 Markdown 引文块；其余情况一律用无引号的概括表达。\n"
