...
def detect_year_header(
    ...
    for row in worksheet.iter_rows(
        min_row=1,
        max_row=min(getattr(worksheet, "max_row", 0), max_scan_rows),
        max_col=min(getattr(worksheet, "max_column", 0), max_scan_cols),
    ):
        matches = [
            {
            ...
            and len(str(cell.value or "").strip()) <= 24
            and pattern.search(str(cell.value or "").strip())
        ]
        if len(matches) > len(best_matches):
            best_row, best_matches = int(row[0].row), matches
    return {"row": best_row, "columns": best_matches} if len(best_matches) >= 2 else {}


def collect_formula_examples(
    worksheet: Any, *, max_formula_examples: int
) -> Dict[str, Any]:
    formulas, external = [], []
    ...
    for row in worksheet.iter_rows():
        for cell in row:
            if getattr(cell, "data_type", "") != "f":
                continue
            formula_count += 1
            item = {"cell": cell.coordinate, "formula": str(cell.value or "")[:200]}
            if len(formulas) < max_formula_examples:
                formulas.append(item)
            if "[" in str(cell.value or ""):
                external_count += 1
                if len(external) < max_formula_examples:
                    external.append(item)
    return {
        "formula_count": formula_count,
        "formula_examples": formulas,
        "external_formula_count": external_count,
        "external_formula_examples": external,
    }


def row_label_for_year_series(
    worksheet: Any, row_index: int, *, before_column: int
) -> str:
    labels = [
        str(worksheet.cell(row=row_index, column=index).value or "").strip()
        for index in range(1, max(before_column, 1))
    ]
    return next((label for label in reversed(labels) if label), "")


def severity_for_financial_label(
    label: str, high_priority_hints: tuple[str, ...]
) -> str:
...
def display_series_value(value: Any, formula_text: str) -> Any:
    return (
        value if value not in (None, "") else formula_text[:160] if formula_text else ""
    )
