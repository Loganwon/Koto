...
class FileTaskRuntime:
            ...
            for event in file_task_doc_annotate_bridge.stream_request(
                ...
                if event.type == "run.finished":
                    ...
                    terminal_summary = str(
                        terminal_payload.get("summary") or ""
                    ).strip()
                    runtime_payload = (
                        terminal_payload.get("runtime")
                        if isinstance(terminal_payload.get("runtime"), dict)
                        else {}
                    )
                    terminal_status = str(
                        runtime_payload.get("terminal_status") or ""
                    ).strip().lower() or (
                        "verified"
                        if terminal_payload.get("completed_task")
    ...
    def _infer_output_mode(
        ...
        *,
        ...
        if explicit_mode:
            if (
                explicit_mode == "answer"
                and (write_intent or docx_annotation_request)
    ...
    def _classify_request(
        ...
        recipe_candidates = recipe_matches(
            classification_request, files, write_intent=write_intent
        )
    ...
    def _should_adjudicate_intent(
        ...
        if bool(options.get("disable_ai_intent_adjudicator")):
            return False
        if any(key in options for key in ("planner_backend", "planner_policy", "planner_command")):
            return False
    ...
    def _generate_financial_workbook_chart(
        ...
        try:
            ...
            fig.suptitle(
                f"{sheet_name} 财务预测质量检查", fontsize=15, fontweight="bold"
            )
            ...
            plotted = 0
            plotted += plot_lines(
                axes[0][0], monetary, "收入、毛利与利润", "人民币万元"
            )
            plotted += plot_lines(
                axes[0][1], rates, "增长率与利润率", "百分比", as_percent=True
            )
    ...
    def _financial_series_movements(
        ...
        for name, values in series.items():
            ...
            if is_rate:
                ...
                change_text = (
                    f"变化 {((last_number - first_number) * 100):.1f} 个百分点"
                )
    ...
    def _financial_report_executive_summary(
        ...
        return lines[:6] or [
            "已识别关键财务预测指标，但仍需补充底层假设说明后才能形成投资判断。"
        ]
    ...
    def _financial_report_problem_paragraphs(
        ...
        paragraphs: List[Dict[str, str]] = [
            ...
            {
                "text": str(
                    chart_result.get("caption")
                    or "已基于附件 Excel 财务模型生成图表和问题清单。"
                )
            },
        ...
        if external_count:
            issues.append(
                f"工作簿检测到 {external_count} 个外部链接，模型复算依赖外部文件，需补齐底稿或解除外链。"
            )
        formula_count = inspect_data.get("total_formula_cells")
        if formula_count:
            issues.append(
                f"工作簿共检测到 {formula_count} 个公式单元格，建议重点核查关键输出行的公式连续性。"
            )
        issues.extend(
            str(item)
            for item in chart_result.get("issues") or []
        ...
        if not chart_result.get("success"):
            issues.append(
                f"图表生成未完全成功：{chart_result.get('error') or '缺少可作图数据'}。"
            )
        if not issues:
            issues.append(
                "未发现明显结构性红旗；仍建议核对关键假设、外部链接和历史口径。"
            )
        ...
        paragraphs.append(
            {"text": chart_result.get("summary") or "图表已按模型中的关键指标生成。"}
        )
        return paragraphs
    ...
    def _duplicate_supervisor_retry_message(
        ...
        lines.append(
            "只有在本轮已经产生真实文件变更，或任务确实是只读答复时，才允许结束。"
        )
        return "\n".join(lines)
        ...
        if target_type in {"docx", "doc"} and self._looks_like_docx_report_request(
            ...
            criteria.append(
                self._quality_gate_result(
                    ...
                    passed=(
                        "write_docx_content" in operations and paragraphs_written >= 3
                    )
                    or paragraphs_written >= 3,
    ...
    def _verify_task(
        ...
        if runtime_status == "awaiting_confirmation":
            ...
            return {
                ...
                "summary": str(
                    tool_runtime_outcome.get("summary")
                    or "任务已暂停，等待用户确认继续。"
                ),
                "remaining": remaining,
                "next_action_artifact": artifact,
