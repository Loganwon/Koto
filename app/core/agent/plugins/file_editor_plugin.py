...
class FileEditorPlugin(AgentPlugin):
    ...
    def get_tools(self) -> List[Dict[str, Any]]:
        return [
            ...
            # — 精准编辑首选（无需预先读文件）—
            {
                ...
                "description": "[PREFERRED for targeted edits] Replace the FIRST occurrence of exact text in a file. No need to read the file first. Use for single-location changes.",
            },
            {
                "name": "patch_file",
                "func": self.patch_file,
                "description": '[PREFERRED for multi-location edits] Apply multiple text replacements to a file in a SINGLE call. Pass a list of {"old": "...", "new": "..."} pairs. Much faster than calling replace_text repeatedly. No need to read the file first.',
            },
            {
                "name": "insert_line",
                "func": self.insert_line,
                "description": 'Insert a line at a specific 1-based line number (mode="before"|"after"). No need to read the file first.',
            },
            {
                "name": "delete_lines",
                "func": self.delete_lines,
                "description": "Delete lines start_line..end_line (1-based, inclusive). No need to read the file first.",
            },
            {
                "name": "append_text",
                "func": self.append_text,
                "description": "Append text to the end of a file.",
            },
            {
                "name": "write_file",
                "func": self.write_file,
                "description": "[FULL OVERWRITE] Write complete content to a file. Use ONLY for creating a new file or completely rewriting a file. For partial edits use replace_text or patch_file instead.",
            },
            {
                "name": "delete_file",
                "func": self.delete_file,
                "description": "Delete a file (auto-backed up before deletion).",
            },
            {
                "name": "copy_file",
                "func": self.copy_file,
            ...
            },
            {
                "name": "move_file",
                "func": self.move_file,
                "description": "Move or cut a file to a new location.",
            },
            {
                "name": "rename_file",
            ...
            },
            {
                ...
                "description": "Create a directory (including all parent dirs).",
            },
            {
                "name": "get_file_info",
            ...
            },
            {
                "name": "find_file",
                "func": self.find_file,
                "description": "Search for files by name keyword on the local system.",
            },
            {
                "name": "list_backups",
                "func": self.list_backups,
                "description": "List all auto-created file backups.",
            },
            {
                "name": "restore_backup",
                "func": self.restore_backup,
    ...
    def insert_line(
        ...
        return (
            f"Line inserted in {file_path}" if r["success"] else f"Error: {r['error']}"
        )
    ...
    def delete_file(self, file_path: str) -> str:
        ...
        return (
            r.get("message", f"Deleted {file_path}")
            if r["success"]
            else f"Error: {r['error']}"
        )
    ...
    def get_file_info(self, file_path: str) -> str:
        ...
        if r["is_dir"]:
            lines.append(
                f"Children: {r.get('children_count', '?')} ({r.get('files_count', 0)} files, {r.get('dirs_count', 0)} dirs)"
            )
        return "\n".join(lines)

    def list_directory(
        self, directory: str, recursive: bool = False, max_items: int = 50
    ) -> str:
        r = self.service.list_directory(
            directory, recursive=recursive, max_items=max_items
        )
        if not r["success"]:
            return f"Error: {r['error']}"
        lines = [
            f"Directory: {r['directory']} ({r['count']} items"
        ...
        for item in r["items"]:
            ...
            lines.append(
                f"  {icon} {item['name']}{size}  {item['modified']}  {item['path']}"
            )
        return "\n".join(lines)
    ...
    def find_file(
        ...
        r = self.service.search_files(
            query, search_dir=search_dir, file_type=file_type, max_results=max_results
        )
        if not r["success"]:
            return f"Error: {r['error']}"
        if not r["results"]:
        ...
        for item in r["results"]:
            lines.append(
                f"  📄 {item['name']}  [{item.get('size_human', '')}]  {item.get('modified', '')}  {item['path']}"
            )
        return "\n".join(lines)
    ...
    def list_backups(self) -> str:
        ...
        for b in r["backups"]:
            lines.append(
                f"  {b['name']}  [{b['size_human']}]  {b['created']}  {b['path']}"
            )
        return "\n".join(lines)
