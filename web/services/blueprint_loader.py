...
def register_all_blueprints(app: Flask, logger: Logger):
    """
    Register all API and web blueprints on the Flask app.

    This wraps the deferred blueprint registration logic in a single callable
    that can be invoked during app initialization.

    Returns the agent blueprint (if available) for WebSocket wiring.
    """
    ...
    def _safe_preload(mod_name: str) -> None:
        try:
            ...
            _mod_logging.getLogger(__name__).warning(
