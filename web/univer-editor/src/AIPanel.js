...
export class AIPanel {
  ...
  _render() {
    ...
    this._container.innerHTML = `
      ...
      <div class="ai-actions">
        ...
        <button class="action-btn" data-action="rewrite"         title="AI 改写">✏️ 改写</button>
        <button class="action-btn" data-action="summarize"       title="生成全文摘要">📝 摘要</button>
        <button class="action-btn" data-action="continue_writing" title="AI 续写">🔊 续写</button>
        <button class="action-btn" data-action="annotate"        title="AI 标注">🔖 标注</button>
      </div>
  ...
  _onAction(actionType) {
    const fullDocActions = ['summarize', 'continue_writing'];
    const selectionActions = ['polish', 'translate', 'rewrite', 'annotate'];

    if (fullDocActions.includes(actionType)) {
      const fullText = this._doc.getFullText();
      if (!fullText.trim()) {
        this.addMessage('文档为空，请先输入内容。', 'error');
        return;
      }
      const labels = { summarize: '📝 全文摘要', continue_writing: '🔊 AI 续写' };
      this.addMessage(labels[actionType] || actionType, 'user');
      this._bridge.sendAction(actionType, { text: fullText, range: null });
      this.expand();
      return;
    }

    if (selectionActions.includes(actionType)) {
