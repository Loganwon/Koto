...
def _build_file_task_orchestrator(*, workspace_root, gemini_client):
    ...
    from app.core.agent.file_task_intent_classifier import QwenIntentClassifier
    from app.core.agent.file_task_multitarget import MultiTargetOrchestrator

    model_client = FileTaskModelClient()
    intent_classifier = QwenIntentClassifier(model_client=model_client)
    ...
    return MultiTargetOrchestrator(runtime=runtime)
...
def chat_stream():
    ...
    _local_chat_override = (
        False  # True → 跳过 is_simple_query()，直接走本地 Ollama 通道
    )
    try:
                ...
                if use_local_planner:
                    ...
                    if _lp_status in ("fail", "partial") and _lp_next:
                        ...
                        try:
                            ...
                            _app_logger.warning(
                                f"[MULTI_STEP] ⚠️ 自检补充执行失败: {_rp_err}"
                            )
        ...
        try:
            ...
            if task_type == "MEETING_EXTRACT":
                ...
                if len(_meeting_text) < 30:
                    ...
                    session_manager.append_and_save(
                        f"{session_name}.json", user_input, _hint
                    )
                    total_time = time.time() - start_time
                    yield f"data: {json.dumps({'type': 'done', 'images': [], 'saved_files': [], 'total_time': total_time})}\n\n"
                    return
...
"""
                try:
                    ...
                    _decisions = (
                        _parsed.get("decisions")
                        if isinstance(_parsed.get("decisions"), list)
                        else []
                    )
                    _actions = (
                        _parsed.get("action_items")
                        if isinstance(_parsed.get("action_items"), list)
                        else []
                    )
                    ...
                    if _actions:
                        ...
                        for _item in _actions[:12]:
                            if not isinstance(_item, dict):
                                continue
                            _prio = _p_map.get(
                                str(_item.get("priority") or "medium").lower(), "中"
                            )
                        ...
                        try:
                            ...
                            for _sk_id in _sk_matched:
                                ...
                                if (
                                    ...
                                    _app_logger.debug(
                                        f"[DOC_ANNOTATE] 注入 Skill: {_sk_id}"
                                    )
                                    break
...
@app.route("/api/editor/ai/task-stream/cancel", methods=["POST"])
