...
直接关闭将丢失修改，是否继续？`);
  ...
  function showMiniGame() {
    if (window.enableMiniGame === false) return;
  ...
  function _setMcpCardState(index, state) {
    const cards = document.querySelectorAll("#mcpSettingsStatusCards .mcp-status-card");
  ...
  async function refreshMcpSettingsStatus() {
    const serviceText = document.getElementById("mcpServiceStatusText");
    const toolText = document.getElementById("mcpToolStatusText");
    const agentText = document.getElementById("mcpAgentStatusText");
    const sessionsEl = document.getElementById("mcpExternalSessions");
    const hintEl = document.getElementById("mcpStatusText");
    if (!serviceText && !toolText && !agentText && !sessionsEl) return;
    _setMcpCardState(0, "checking");
    _setMcpCardState(1, "checking");
    _setMcpCardState(2, "checking");
    if (serviceText) serviceText.textContent = "检测中";
    if (toolText) toolText.textContent = "检测中";
    if (agentText) agentText.textContent = "检测中";
    try {
      const response = await fetch("/api/mcp/status", { headers: { Accept: "application/json" } });
      const data = await response.json();
      if (!response.ok || data.success !== true) throw new Error(data.error || `HTTP ${response.status}`);
      const toolCount = Number(data.exposed_tool_count ?? 0);
      const websocket = data.websocket || {};
      const externalCount = Number(websocket.active_external_session_count ?? 0);
      const sessions = Array.isArray(websocket.external_sessions) ? websocket.external_sessions : [];
      _setMcpCardState(0, "online");
      _setMcpCardState(1, toolCount > 0 ? "online" : "idle");
      _setMcpCardState(2, externalCount > 0 ? "online" : "idle");
      if (serviceText) serviceText.textContent = "已启动";
      if (toolText) toolText.textContent = `${toolCount} 个工具`;
      if (agentText) agentText.textContent = externalCount > 0 ? `${externalCount} 个已连接` : "等待连接";
      if (sessionsEl) {
        sessionsEl.innerHTML = sessions.length ? sessions.map((item) => {
          const name = escapeHtml(item.client_name || item.id || "unknown");
          const version = item.client_version ? ` v${escapeHtml(item.client_version)}` : "";
          const seen = _formatSessionTime(item.last_seen);
          return `<div class="mcp-session-item"><span class="mcp-session-name">${name}${version}</span><span class="mcp-session-meta">${seen ? `最近 ${seen}` : "已连接"}</span></div>`;
        }).join("") : "暂无外部 Agent 连接";
      }
      if (hintEl) hintEl.textContent = `JSON-RPC 与 WebSocket 均由后端确认；外部 Agent 数量不包含当前 Koto 页面。`;
    } catch (error) {
      _setMcpCardState(0, "offline");
      _setMcpCardState(1, "offline");
      _setMcpCardState(2, "offline");
      if (serviceText) serviceText.textContent = "不可用";
      if (toolText) toolText.textContent = "未检测";
      if (agentText) agentText.textContent = "未检测";
      if (sessionsEl) sessionsEl.textContent = "无法读取 MCP 后端状态";
      if (hintEl) hintEl.textContent = `状态读取失败：${error?.message || error}`;
    }
  }
  ...
  async function updateSetting(section, key, value) {
    try {
      const response = await csrfFetch("/api/settings", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ category: section, section, key, value })
      });
      const data = await response.json().catch(() => ({}));
      ...
      if (currentSettings) {
        const bucket = currentSettings[section] || (currentSettings[section] = {});
        bucket[key] = value;
        window.currentSettings = currentSettings;
      }
    } catch (e) {
      console.warn("[Settings] 保存设置失败:", section, key, e?.message || e);
      if (typeof window.showNotification === "function") {
        window.showNotification(`设置保存失败：${e?.message || e}`, "error", 2500);
      }
    }
  }
  async function onMiniGameChange(enabled) {
    window.enableMiniGame = enabled;
    await updateSetting("ai", "enable_mini_game", enabled);
  }
  ...
  function copyMcpText(elementId, label = "内容") {
    const text = document.getElementById(elementId)?.textContent?.trim() || "";
    if (!text) return;
    navigator.clipboard.writeText(text).then(() => {
      if (typeof window.showNotification === "function") {
        window.showNotification(`已复制${label}`, "success", 1500);
      }
    }).catch(() => {
  ...
  window.refreshMcpSettingsStatus = refreshMcpSettingsStatus;
  window.copyMcpText = copyMcpText;
  ...
  window.onMiniGameChange = onMiniGameChange;
