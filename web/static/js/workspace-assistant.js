...
(function() {
  ...
  const state = {
    ...
    lockedModel: localStorage.getItem('wa_locked_model') === 'local' ? 'local' : 'cloud',
  ...
  function _renderAIFileChips() {
    ...
    list.innerHTML = state._aiFileContext.map((f, i) => {
      ...
      return `<div class="wa-ctx-file-row${isTarget ? ' ai-target' : ''}${isLoading ? ' loading' : ''}${hasError ? ' error' : ''}" title="${_escHtml(rowTitle)}">` +
  ...
  function _restoreDefaultQuickActions() {
    ...
    bar.innerHTML =
      `<button type="button" class="wa-quick-btn" onclick="WA.sendQuickAction('润色')" title="润色选区；未选择时处理当前文件">润色表达</button>` +
      `<button type="button" class="wa-quick-btn" onclick="WA.sendQuickAction('翻译')" title="翻译选区；未选择时处理当前文件">翻译内容</button>` +
      `<button type="button" class="wa-quick-btn" onclick="WA.sendQuickAction('总结')" title="总结选区；未选择时处理当前文件">提炼要点</button>` +
      `<button type="button" class="wa-quick-btn" onclick="WA.sendQuickAction('检查')" title="检查选区；未选择时处理当前文件">检查问题</button>` +
      `<button type="button" class="wa-quick-btn chart-btn" onclick="WA.sendQuickAction('可视化')" title="将选中数据用 Python 可视化为图表">可视化</button>`;
  }
  ...
  function _serializeReviewProposal(proposal) {
    ...
    raw.source = String(raw.source || '').trim() || 'user_revision';
  ...
  function _renderReviewShell() {
    ...
    if (!sections.length) {
      ...
      const preserveCommentModeEmptyState = _isReviewCommentModeEnabled() && !reviewState.comments.length;
  ...
  window.WA.deleteSelected = async () => {
    ...
    showToast(failed ? `已移入回收站 ${paths.length - failed} 个，${failed} 个失败` : `已将 ${paths.length} 个文件移入回收站`, failed ? 'error' : 'success');
    WA.toggleSelectMode();
    await loadWorkspaceFiles();
  };
  ...
  window.WA.stopStream = () => {
    ...
    if (runId && window.WA && typeof window.WA.cancelFileTaskRun === 'function') {
      Promise.resolve(window.WA.cancelFileTaskRun(runId)).catch((error) => {
  ...
  window.WA.sendMessage = () => {
      ...
      const liveSelection = !pinnedSel ? _getLiveEditorSelectionForAI() : null;
  ...
  function _selectedCloudModelId() {
    return (state.lockedModel && !['cloud', 'local'].includes(state.lockedModel))
      ? state.lockedModel
      : '';
  }
  ...
  function _syncModelStatusUi() {
    ...
    if (cloudModelEl) {
      cloudModelEl.textContent = `文件: ${cloudModelHint}`;
      cloudModelEl.title = `云端文件任务模型：${cloudModelHint}`;
      cloudModelEl.hidden = !(shouldShowModelHint && activeMode === 'cloud');
    }
    if (localModelEl) {
  ...
  function _syncLockedModelFromServer() {
    return fetch('/api/local-model/status', { cache: 'no-store' })
      ...
      .then(data => {
        ...
        const serverLockedModel = data.mode === 'local' ? 'local' : 'cloud';
  ...
  window.WA.setUseLocalModel = (useLocal) => {
    const newModel = useLocal ? 'local' : 'cloud';
  ...
  window.WA.setLockedModel = (val) => {
    const normalized = String(val || '').trim().toLowerCase() === 'local' ? 'local' : 'cloud';
    window.WA.setUseLocalModel(normalized === 'local');
  };
