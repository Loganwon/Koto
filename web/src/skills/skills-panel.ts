...
const _SP_FILE_SKILL_IDS = new Set<string>([
  'pdf_reader', 'multi_format_reader', 'long_doc_parser', 'spreadsheet_analyst',
  'table_extractor', 'cross_format_extractor', 'multi_doc_synthesis',
  'doc_format_fixer', 'doc_structure_optimizer', 'table_enhancer',
  'doc_tone_adjuster', 'doc_fact_checker', 'doc_readability', 'doc_dedup',
  'legal_doc_review', 'financial_doc_review', 'academic_paper_polish',
  'marketing_copy', 'excel_formula_expert', 'excel_data_cleaner', 'pivot_advisor',
  'slide_storyteller', 'slide_data_viz', 'doc_smart_compare',
  'questionnaire_filler', 'comm_digest', 'data_format_cleaner',
  'ppt_generator_pro', 'excel_generator_pro', 'docx_generator_pro',
  'pdf_generator_pro', 'docx_translator'
]);
let _spCurrentSearch = '';
let _spCurrentSort = 'default';
let _spSelectedSessionId: string | null = null;
let _spCommunitySkills: CommunitySkillItem[] = [];
let _spCommunityLoading = false;
let _spCommunitySearch = '';

const CAT_LABELS: Record<string, string> = { behavior: '⚙️ 行为', style: '🎨 风格', domain: '🔬 领域', custom: '🔧 自定义' };
...
function _isUnifiedWorkspace(): boolean {
...
(window as any).openSkillsPanel = function () {
  const panel = document.getElementById('skillsPanel');
  if (!panel) return;
  if (typeof (window as any).closeSettings === 'function') (window as any).closeSettings();
...
(window as any).closeSkillsPanel = function () {
  const panel = document.getElementById('skillsPanel');
  if (!panel) return;
...
(window as any).toggleSkillsPanel = function () {
  const panel = document.getElementById('skillsPanel');
  if (!panel) return;
  if (panel.classList.contains('active')) {
    (window as any).closeSkillsPanel();
  } else {
    (window as any).openSkillsPanel();
  }
};

// CoworkPanel
(window as any).CoworkPanel = (() => {
  ...
  function _phaseLabel(phase: string): string {
    const map: Record<string, string> = { planning: '规划中', review: '待审批', executing: '执行中', done: '已完成', failed: '失败' };
  ...
  function _toolIcon(hint: string): string {
    const m: Record<string, string> = { web_search: '🌐', read_file: '📄', write_file: '💾', code_exec: '⚙️', memory_search: '🧠' };
    return m[hint] || '🔧';
  }

  function _md(text: string): string {
    if (typeof (window as any).marked !== 'undefined') {
      try { return (window as any).marked.parse(text || ''); } catch (e) { }
    }
    return (text || '').replace(/\n/g, '<br>');
  }
  ...
  async function submit(): Promise<void> {
    const goal = ((_el('cwGoalInput') as HTMLInputElement)?.value || '').trim();
    if (!goal) { _el('cwGoalInput')?.focus(); return; }
    const context = ((_el('cwContextInput') as HTMLInputElement)?.value || '').trim();
    const human_review = (_el('cwHumanReviewToggle') as HTMLInputElement)?.checked || false;

    (_el('cwSubmitBtn') as HTMLButtonElement)!.disabled = true;
    (_el('cwSubmitBtn') as HTMLButtonElement)!.textContent = '提交中…';
    ...
    try {
      ...
      (_el('cwSubmitBtn') as HTMLButtonElement)!.disabled = false;
      (_el('cwSubmitBtn') as HTMLButtonElement)!.textContent = '▶ 开始执行任务';
    }
  }

  function _startMonitor(taskId: string): void {
    _stopMonitor();
    try {
      _sse = new EventSource('/api/bg-agent/' + taskId + '/stream');
      _sse.onmessage = (ev) => {
        try { _handleUpdate(JSON.parse(ev.data)); } catch (e) { }
      };
      _sse.onerror = () => {
  ...
  function _startPoll(taskId: string): void {
    _pollTimer = setInterval(() => _fetchStatus(taskId), 2500);
  }

  function _stopMonitor(): void {
    if (_sse) { try { _sse.close(); } catch (e) { } _sse = null; }
    if (_pollTimer) { clearInterval(_pollTimer); _pollTimer = null; }
  }

  async function _fetchStatus(taskId: string): Promise<void> {
    try {
  ...
  function _handleUpdate(status: any): void {
    if (!status) return;
  ...
  function _renderPlan(plan: any): void {
    if (_el('cwPlanReasoning')) _el('cwPlanReasoning')!.textContent = plan.reasoning || '';
    if (_el('cwPlanMeta')) _el('cwPlanMeta')!.textContent =
      '预计耗时: ' + (plan.estimated_minutes || '?') + ' 分钟 · ' + (plan.steps || []).length + ' 个步骤';
    const container = _el('cwPlanSteps');
    if (!container) return;
    container.innerHTML = (plan.steps || []).map((s: any, i: number) =>
      `<div class="cw-step">
  ...
  function _renderExecSteps(steps: any[], currentStepId: string): void {
    const container = _el('cwExecSteps');
    if (!container) return;
    container.innerHTML = (steps || []).map((s: any) => {
  ...
  async function reject(): Promise<void> {
    if (!_taskId) return;
    const feedback = (_el('cwRejectFeedback') as HTMLInputElement)?.value || '';
    try {
      ...
      _updateBadge(0);
      newTask();
    } catch (e: any) { alert('拒绝失败: ' + e.message); }
  }
  ...
  function newTask(): void {
    _taskId = null;
    _stopMonitor();
    const goalInput = _el('cwGoalInput') as HTMLInputElement;
    if (goalInput) goalInput.value = '';
    const contextInput = _el('cwContextInput') as HTMLInputElement;
  ...
  async function _loadHistory(): Promise<void> {
    ...
    try {
      ...
      if (!tasks.length) { container.innerHTML = '<div class="cw-empty">暂无历史任务</div>'; return; }
      container.innerHTML = tasks.map((t: any) => {
        const dt = t.submitted_at ? new Date(t.submitted_at * 1000).toLocaleString('zh-CN', { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '';
...
async function spLoadSkills(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (cres.ok) {
        ...
        _spSuppressedIds = new Set((cdata.conflicts || []).map((c: ConflictData) => c.loser_id));
        _spConflictWinner = {};
...
function spUpdateActiveBadge(): void {
...
function spRenderCards(): void {
  ...
  if (!skills.length) {
    if (tab === 'library') {
      ...
      content.innerHTML = `<div class="sp-loading">本地无匹配结果</div><div id="spLocalSearchCommunity" style="margin-top:10px;"></div>`;
                    (window as any).spSearchCommunityFallback(q);
    } else {
  ...
  const CAT_SHORT: Record<string, string> = { behavior: '行为', style: '风格', domain: '领域', custom: '自定义', workflow: '工作流' };

  function renderTile(s: SkillManifest): string {
  ...
  if (tab === 'catalog' && cat === 'custom') {
    ...
    (studioBanner as any).onclick = () => (window as any).spSwitchTab('studio');
  ...
  if (tab === 'catalog' && !q && cat === 'all') {
    ...
    (banner as any).onclick = () => (window as any).spSwitchTab('community');
...
async function spLoadCommunitySkills(): Promise<void> {
...
function spSkillStats(id: string): { likes: number; installs: number } {
  let h = 0;
...
function spRenderCommunityCards(): void {
  const content = document.getElementById('spCommunityContent');
  if (!content) return;
  ...
  const CAT_LABEL: Record<string, string> = { behavior: '行为', style: '风格', domain: '领域', coding: '代码', writing: '写作', career: '职场', research: '研究', lifestyle: '生活', custom: '自定义', workflow: '工作流' };
  function renderCard(s: CommunitySkillItem): string {
...
(window as any).spToggleCommunityCard = function (skillId: string) {
  const card = document.getElementById('spCommCard_' + skillId);
  if (!card) return;
  card.classList.toggle('open');
};

(window as any).spInstallCommunitySkill = async function (skillId: string, isInstalled: boolean) {
  if (isInstalled) {
    if ((window as any).toast) (window as any).toast('该 Skill 已安装，可在「我的库」中管理');
    return;
  }
  const card = document.getElementById('spCommCard_' + skillId);
  ...
  try {
    ...
    if (r.ok) {
      if ((window as any).toast) (window as any).toast('安装成功，已为您启用该 Skill！');
      ...
      const data = await r.json();
      if ((window as any).toast) (window as any).toast('安装失败: ' + (data.error || '未知错误'), 'error');
    }
  } catch (e) {
    if ((window as any).toast) (window as any).toast('网络错误，请稍后重试', 'error');
  }
};

(window as any).spPreviewCommunitySkill = async function (skillId: string) {
  ...
  const catText: Record<string, string> = { behavior: '行为增强', domain: '专业领域', coding: '代码工程', writing: '写作创作', career: '职场效率', research: '研究分析', lifestyle: '生活助手' };
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (box) box.textContent = (skill.prompt || skill.system_prompt || '（无法加载）') as string;
    }
  } catch (e) {
    const box = document.getElementById('spPreviewPromptBox');
    if (box) box.textContent = (skill.prompt || skill.system_prompt || '（网络错误）') as string;
  }
};

(window as any).spSearchCommunityFallback = async function (q: string) {
  ...
  const matched = _spCommunitySkills.filter(s =>
    !s.is_installed &&
    (s.name.toLowerCase().includes(qLower) || (s.description && s.description.toLowerCase().includes(qLower)))
  ).slice(0, 5);
...
window.spSwitchTab = function (tab: string) {
  ...
  if (tab === 'studio') {
    ...
    if (community) { community.style.display = 'flex'; spLoadCommunitySkills(); }
  } else {
...
(window as any).spToggleSkill = async function (skillId: string, enabled: boolean) {
  ...
  spUpdateActiveBadge();
  if (typeof (window as any).toggleSkill === 'function') {
    await (window as any).toggleSkill(skillId, enabled);
  }
  spRenderCards();
};
...
async function spLoadSessionList(): Promise<void> {
  ...
  try {
    ...
    list.innerHTML = sessions.map((s: any) => `
...
(window as any).spSelectSession = function (sessionId: string, el: HTMLElement) {
...
function spWizardGoToStep(n: number): void {
  for (let i = 1; i <= 3; i++) {
...
window.spExtractFromSession = async function () {
  ...
  const resultEl = document.getElementById('spExtractResult') as HTMLElement;
...
// ── Hook ─────────────────────────────────────────────────────
const _origRefreshActiveSkills = (window as any).refreshActiveSkills;
(window as any).refreshActiveSkills = async function () {
...
(window as any).getSpSkills = function () { return _spSkills; };
...
function spGetUserPresets(): PresetItem[] {
...
(window as any).spDeletePreset = function (id: string) {
  const presets = spGetUserPresets().filter(p => p.id !== id);
  spSaveUserPresets(presets);
  spRenderPresets();
};

(window as any).spApplyPreset = async function (preset: PresetItem) {
  ...
  for (const c of changes) {
    if (typeof (window as any).toggleSkill === 'function') {
      await (window as any).toggleSkill(c.id, c.target);
    }
...
const _spPresetRegistry: Record<string, PresetItem> = {};

(window as any).spApplyPresetById = function (id: string) {
  const p = _spPresetRegistry[id];
  if (p) (window as any).spApplyPreset(p);
};

function spRenderPresets(): void {
  ...
  function renderPresetCard(p: PresetItem): string {
    ...
    const delBtn = !p.is_builtin
      ? `<button class="sp-preset-del-btn" onclick="event.stopPropagation();spDeletePreset('${p.id}')" title="删除"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>`
      : '';
...
(window as any).spTrackMessageUsage = function () {
  const now = Date.now();
...
async function spLoadRecStrip(): Promise<void> {
  ...
  try {
    ...
    chips.innerHTML = data.skills.map((s: SkillManifest) => {
...
window.spAskKoto = async function () {
  ...
  const reasoningEl = document.getElementById('spAkReasoning') as HTMLElement;
  const cardsEl = document.getElementById('spAkCards') as HTMLElement;
  const task = (taskInput ? taskInput.value : '').trim();
  if (!task) { taskInput && taskInput.focus(); return; }
  ...
  try {
    ...
    if (!data.skills || !data.skills.length) {
      ...
      cardsEl!.innerHTML = data.skills.map((s: SkillManifest) => {
...
(window as any).spAkToggle = async function (skillId: string, btnEl: HTMLElement) {
  const skill = _spSkills.find(s => s.id === skillId);
  const toEnable = !(skill ? skill.enabled : btnEl.classList.contains('enabled'));
  btnEl.textContent = '…';
  await (window as any).spToggleSkill(skillId, toEnable);
  btnEl.classList.toggle('enabled', toEnable);
  btnEl.textContent = toEnable ? '✓ 已启用' : '启用';
};

(window as any).spRenderCards = spRenderCards;
(window as any).spLoadSkills = spLoadSkills;

(window as any).spGhInstallUrl = async function () {
  const input = document.getElementById('spGhUrlInput') as HTMLInputElement;
  const status = document.getElementById('spGhInstallStatus') as HTMLElement;
  if (!input || !status) return;
  const rawUrl = (input.value || '').trim();
  if (!rawUrl) { input.focus(); return; }
...
// Pre-load skills silently on page init
(async function () {
  try {
    const resp = await fetch('/api/skills');
    if (!resp.ok) return;
