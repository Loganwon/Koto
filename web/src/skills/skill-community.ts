...
// ── DOM refs ─────────────────────────────────────────
const $ = function (id: string): HTMLElement | null { return document.getElementById(id); };
const searchInput = $("sc-search") as HTMLInputElement | null;
...
const aiInput = $("sc-ai-input") as HTMLInputElement | null;
...
// ── State ────────────────────────────────────────────
let allSkills: CommunitySkill[] = [];
let currentCat = "all";
let currentSort = "default";
let currentSearch = "";
let modalSkill: CommunitySkill | null = null;
...
function updateStats(): void {
  ...
  const counts: Record<string, number> = { all: allSkills.length, behavior: 0, domain: 0, coding: 0 };
...
function renderGrid(): void {
  ...
  const catNames: Record<string, string> = { behavior: "思维增强", domain: "专业领域", coding: "代码工程", other: "其他" };
  let html = "";
...
// ── Card HTML ───────────────────────────────────────
function renderCard(s: CommunitySkill): string {
...
// ── Quick install ──────────────────────────────────
function quickInstall(skillId: string, btnEl: HTMLElement): void {
...
// ── Online install ─────────────────────────────────
function onlineInstall(resultIndex: string, btnEl: HTMLElement): void {
  if (btnEl.classList.contains("done") || btnEl.classList.contains("busy")) return;
  const idx = parseInt(resultIndex, 10);
  const result = (window as any)._aiResults && (window as any)._aiResults[idx];
  if (!result) return;
...
function markInstalled(skillId: string): void {
  for (let i = 0; i < allSkills.length; i++) {
...
function markUninstalled(skillId: string): void {
  for (let i = 0; i < allSkills.length; i++) {
    if (allSkills[i].id === skillId) {
...
function doAiRecommend(): void {
  ...
  csrfFetch(API + "/community/ai-recommend", {
    ...
    .then(function (d: any) {
      ...
      (window as any)._aiResults = results;
      if (results.length === 0) {
...
function renderAiResults(results: any[]): void {
  let html = "";
...
function openDetail(skillId: string): void {
  let local: CommunitySkill | null = null;
  for (let i = 0; i < allSkills.length; i++) {
...
// ── Install (from modal) ───────────────────────────
function doInstall(skill: CommunitySkill): void {
  if (mInstall.classList.contains("done")) return;
  mInstall.textContent = "安装中…";
  (mInstall as HTMLButtonElement).disabled = true;
...
function doUninstall(skill: CommunitySkill): void {
...
function bindEvents(): void {
  ...
  gridEl.addEventListener("click", function (e) {
    ...
    const card = (e.target as HTMLElement).closest(".sc-card") as HTMLElement | null;
