...
export function _hexLuma(hex: string): number {
...
export function _safeTextColor(fgHex: string, bgHex: string): string | null {
  const fg = String(fgHex || '').trim();
  if (!fg || !/^#?[0-9a-fA-F]{3}([0-9a-fA-F]{3})?$/.test(fg)) return null;
  const bgLuma = _hexLuma(bgHex);
  const fgLuma = _hexLuma(fg);
  return Math.abs(fgLuma - bgLuma) >= 0.32 ? fg : null;
}

export function _runTextDecoration(run: any): string {
  const parts: string[] = [];
