 ...
 * Extracted from workspace-assistant.js lines 1531-2111.
 */

import { _fileIcon, _escHtml, showToast, _FOLDER_OPEN_SVG, _FOLDER_PICK_SVG, _FOLDER_SVG } from './infrastructure';
import { state, _trackUserOpen, loadRecentFiles } from './state';
...
function _fileActionButtons(supported: boolean): string {
  ...
  return (
    ...
    sendButton +
    `<button type="button" class="wa-file-more" onclick="event.stopPropagation();WA._showBrowserCtx(event,this.closest('.wa-file-item'))" title="更多操作" aria-label="更多操作">${_MORE_BTN_SVG}</button>` +
    `</div>`
  );
}
...
function _focusVisibleAIComposer(): void {
  const input = document.getElementById(_isAiSessionListVisible() ? 'wa-session-list-input' : 'wa-user-input') as HTMLInputElement | null;
...
function _searchCachedBrowserEntries(query: string, category: string, limit: number = 60): SearchResult[] {
  ...
  const visit = (entry: BrowserNode | null | undefined) => {
    ...
    if (isFolder) {
      ...
      const cached = state._browserCache[entry.path];
    ...
    await Promise.all(
      expanded.map(async (absPath) => {
        try {
          ...
          if (prevKey !== freshKey) {
            state._browserCache[absPath] = fresh;
            changed = true;
          }
        } catch (_) {
          /* ignore */
  ...
  if (isFolder) {
    ...
    rows.push(
      `<div class="wa-file-item folder" style="padding-left:${pad}px" ` +
        ...
        `<div class="wa-file-actions"><button type="button" class="wa-file-more" onclick="event.stopPropagation();WA._showBrowserCtx(event,this.closest('.wa-file-item'))" title="更多操作" aria-label="更多操作">${_MORE_BTN_SVG}</button></div>` +
        `</div>`
    );
    if (isExpanded) {
...
async function toggleBrowserFolder(absPath: string): Promise<void> {
  ...
  if (!state._browserCache[absPath] || state._browserCache[absPath] === 'loading') {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        showToast(data.error || '无法读取文件夹', 'error');
        state._browserCache[absPath] = [];
      } else {
        state._browserCache[absPath] = data.entries || [];
      }
    } catch (e: any) {
      state._browserCache[absPath] = [];
