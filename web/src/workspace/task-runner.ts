...
function ensureStep(card: TaskCardElement, stepId: string, title: string): HTMLElement {
  ...
  step.innerHTML = '<div class="wa-task-step-head"><span class="wa-task-step-dot"></span><span class="wa-task-step-title">' + esc(title || safeId) + '</span></div><div class="wa-task-step-body"></div>';
...
function taskTerminalResult(card: TaskCardElement, fallbackSummary?: string): TerminalResult {
  ...
  else if (terminalStatus === 'failed' || terminalStatus === 'blocked' || terminalStatus === 'write_blocked' || terminalStatus === 'tool_gap') status = 'error';
...
function terminalStepSummary(result: TerminalResult): string {
  if (!result) return '任务已完成，完整结果见对话汇报。';
  if (result.status === 'error') return '执行失败，详情见对话汇报。';
  if (result.status === 'cancelled') return '任务已取消。';
  if (result.status === 'pending') return '任务等待确认，详情见对话汇报。';
  return '任务已完成，完整结果见对话汇报。';
}
...
function taskResultActionsHtml(card: TaskCardElement): string {
  ...
  const completed = String(dataset.taskCompleted || '').trim().toLowerCase() !== 'false'
    && !['failed', 'blocked', 'write_blocked', 'tool_gap', 'cancelled'].includes(terminal);
...
function notifyTaskWorkbenchForCard(card: TaskCardElement): void {
  if (!isTaskCardElement(card)) return;
  if ((window as any).WA && typeof (window as any).WA.notifyTaskFlowChanged === 'function') {
    (window as any).WA.notifyTaskFlowChanged(card.dataset.taskId || '');
  }
}
...
function renderPlanIntoCard(card: TaskCardElement, data: Record<string, any>): void {
  ...
  if (planEl) {
    const items = steps.map((item, index) => renderPlanStepItem(item, index)).join('');
...
function handleEvent_run_finished(card: TaskCardElement, evt: Record<string, any>, payload: Record<string, any>): void {
  ...
  if (summaryContainer) {
    const visibleSummary = compactFlowSummary(data.summary || data.text || data.error || result.summary || '', terminalStepSummary(result));
    summaryContainer.innerHTML = '<div class="wa-task-plan-summary wa-task-outcome">' + esc(visibleSummary) + '</div>' + taskResultActionsHtml(card);
...
function handleEvent_file_refresh(card: TaskCardElement, evt: Record<string, any>, payload: Record<string, any>): void {
  ...
  if (state.fileRefreshEntries.has(path) && state.fileRefreshEntries.get(path)?.hash === fileRefreshHash) return;
  state.fileRefreshEntries.set(path, { hash: fileRefreshHash || '', nextRefreshTimer: null });
  const fileRefreshController = (window as any).WA?.fileRefreshController?.();
  if (fileRefreshController && typeof fileRefreshController.scheduleRefresh === 'function') {
    fileRefreshController.scheduleRefresh(path, (data as any).file_refresh_hash || '', { delay: 200, force: data.force === true });
  }
}
...
function streamFileTask(cardOrLoadingEl: TaskCardElement | null, url: string, body: Record<string, any> | FormData, method?: string, opts?: Record<string, any>): Promise<TaskCardElement> {
  ...
  if ((window as any).WA?.fileRefreshController) {
    fetchAbort = new AbortController(); signal = fetchAbort.signal;
  }
...
function restoreTaskRunCard(cardEl: TaskCardElement, initialSummary?: string, initialStatus?: string, recoveryPayload?: Record<string, any>): TaskCardElement | null {
  ...
  if (initialSummary && !String(cardEl.dataset.taskSummary || '').trim()) { cardEl.dataset.taskSummary = initialSummary; }
  const stepsHost = cardEl.querySelector('[data-role="steps"]');
  if (stepsHost && !stepsHost.children.length) { stepsHost.innerHTML = '<div class="wa-task-step pending" data-step-id="run"><div class="wa-task-step-head"><span class="wa-task-step-dot"></span><span class="wa-task-step-title">任务状态</span></div><div class="wa-task-step-body"></div></div>'; }
...
function resumePersistedFileTask(optionsOrCard: ResumeFileTaskOptions | TaskCardElement, taskContract?: Record<string, any>): Promise<TaskCardElement> {
  ...
  if (runId) card.dataset.taskRunId = runId;
  restoreTaskRunCard(card, contract.summary || '', options.initialStatus || options.status || contract.initialStatus || contract.status || 'running', {});

  const replay = options.replay === false ? 'false' : 'true';
  const finalUrl = `/api/tasks/${encodeURIComponent(taskId)}/stream?replay=${replay}`;
  return streamFileTask(card, finalUrl, {}, 'GET').then((streamingCard) => {
...
async function streamTaskFlow(optionsOrLoadingEl: StreamFileTaskOptions | TaskCardElement | null, url?: string, body?: Record<string, any> | FormData, method?: string, opts?: Record<string, any>): Promise<any> {
  if (url) {
    return streamFileTask(optionsOrLoadingEl as TaskCardElement | null, url, body || {}, method, opts);
  }
...
WA.streamWhiteboxTask = streamTaskFlow;
WA.streamFileTask = streamFileTask;
