...
export interface TaskDispatcherDeps {
  ...
  openWorkspaceFile?: (path: string) => Promise<any>;
  streamTaskFlow?: (opts: any) => Promise<any>;
  streamWhiteboxTask?: (opts: any) => Promise<any>;
  streamFileTask?: (opts: any) => Promise<any>;
...
export function createTaskDispatcher(deps: TaskDispatcherDeps = {}) {
  ...
  function matchesOpenFileIntent(text: string): boolean {
    return !!openFileMatch(text);
  }

  function openFileTarget(text: string): string {
  ...
  function openFileMatch(text: string): RegExpMatchArray | null {
  ...
  function currentWorkspaceRouteFile(): Record<string, any> | null {
    const activePath = String(state.activeTabPath || state.filePath || state.wsSourcePath || '').trim();
    if (!activePath) return null;
    const tab = Array.isArray(state.openTabs) ? state.openTabs.find((item: any) => item && item.path === activePath) : null;
    return {
      path: previewText(activePath, 260),
      name: previewText((tab && tab.name) || baseNameFromPath(activePath), 180),
      type: previewText((tab && (tab.ext || tab.fileType)) || state.fileType || fileTypeFromPath(activePath), 40),
    };
  }

  function currentWorkspaceTaskFile(): TaskFileInfo | null {
    const currentFile = currentWorkspaceRouteFile();
    if (!currentFile) return null;
    let content = '';
    if (typeof options.getActiveEditorContent === 'function') {
      try { content = String(options.getActiveEditorContent() || ''); } catch { content = ''; }
    }
    return {
      path: String(currentFile.path || ''),
      name: String(currentFile.name || ''),
      type: String(currentFile.type || ''),
      content: typeof options.sampleTaskContext === 'function' ? options.sampleTaskContext(content) : content,
    };
  }

  function mentionsCurrentWorkspaceFile(text: string): boolean {
    const source = String(text || '').trim();
    if (!source) return false;
    return /(?:当前|这个|这份|此|已打开|打开的|正在看的|正在编辑的|active|current|this)\s*(?:文件|文档|表格|演示稿|ppt|pptx|pdf|docx?|xlsx?|txt|md|csv)/i.test(source)
      || /(?:文件|文档|表格|演示稿|ppt|pptx|pdf|docx?|xlsx?|txt|md|csv).{0,8}(?:当前|这个|这份|已打开|打开的|正在看的|正在编辑的|active|current|this)/i.test(source);
  }
  ...
  function shouldExposeCurrentWorkspaceFile(context: TaskContext): boolean {
    const text = String(context && context.text || '').trim();
    if (mentionsCurrentWorkspaceFile(text)) return true;
    return !!String(context && context.pinnedSelText || '').trim();
  }
  ...
  function shouldForceFileTaskForWorkspaceContext(context: TaskContext, routeDecision: Record<string, any> | null): boolean {
    ...
    if (route && route !== 'light_chat' && route !== 'web_search') return false;
    const text = String(context && context.text || '').trim();
    if (!text) return false;
    const hasFileContext = workspaceRouteFiles().length > 0 || !!String(context.pinnedSelText || '').trim() || mentionsCurrentWorkspaceFile(text);
  ...
  function normalizeWorkspaceRouteDecision(data: any): Record<string, any> {
    ...
    const allowed = new Set(['light_chat', 'web_search', 'file_task', 'open_file']);
    const normalizedRoute = allowed.has(route) ? route : 'file_task';
  ...
  function canonicalWorkspaceRouteKind(route: string, routeKind?: string): string {
    ...
    if (normalizedKind === 'direct_response' || normalizedKind === 'complex_task') {
      if (normalizedKind === 'direct_response' && normalizedRoute === 'file_task') return 'complex_task';
      if (normalizedKind === 'complex_task' && ['light_chat', 'web_search', 'open_file'].includes(normalizedRoute)) return 'direct_response';
      return normalizedKind;
    }
    return ['light_chat', 'web_search', 'open_file'].includes(normalizedRoute) ? 'direct_response' : 'complex_task';
  }
  ...
  function canonicalWorkspaceTaskType(route: string, taskType?: string): string {
    ...
    if (normalizedRoute === 'file_task' || normalizedRoute === 'open_file') return 'FILE_TASK';
  ...
  async function resolveWorkspaceRouteIntent(context: TaskContext): Promise<Record<string, any>> {
    const body = {
      ...
      current_file: shouldExposeCurrentWorkspaceFile(context) ? currentWorkspaceRouteFile() : null,
  ...
  function runTaskFlowRoute(context: TaskContext, routeDecision?: Record<string, any>): Promise<any> {
    const loadingEl = context.loadingEl;
    const streamTaskFlow = typeof options.streamTaskFlow === 'function'
  ...
  async function runWorkspaceModelRoutedTask(context: TaskContext): Promise<any> {
    if (shouldBypassWorkspaceRoute(context)) {
      return runTaskFlowRoute(context, {
        route_kind: 'complex_task',
        base_task_type: 'COMPLEX_TASK',
        route: 'file_task',
        task_type: 'FILE_TASK',
        route_source: 'explicit_task_payload',
    ...
    if (shouldForceFileTaskForWorkspaceContext(context, routeDecision)) {
      routeDecision = Object.assign({}, routeDecision || {}, {
        route_kind: 'complex_task',
        base_task_type: 'COMPLEX_TASK',
        route: 'file_task',
        task_type: 'FILE_TASK',
        route_source: 'frontend_file_context_guard',
        keyword_policy: 'hint_only',
      });
    }
    if (routeDecision && routeDecision.route_kind === 'direct_response' && routeDecision.route === 'open_file' && routeDecision.target_path && typeof options.openWorkspaceFile === 'function') {
      const fileToOpen = String(routeDecision.target_path || '').trim();
      try {
        await Promise.resolve(options.openWorkspaceFile(fileToOpen));
        const assistantText = `已打开 ${fileToOpen}`;
        if (context.loadingEl) {
          context.loadingEl.classList.remove('streaming');
          context.loadingEl.textContent = assistantText;
          context.loadingEl.dataset!.rawText = assistantText;
        }
        const metadata = { task_kind: 'message', status: 'done', route_intent: routeDecision, task_request: context.text || fileToOpen };
        appendAssistantConversationTurn(assistantText, metadata);
        persistTaskTurn(context.text || fileToOpen, assistantText, metadata, []);
        state.isLoading = false;
        if (typeof options.setStreamButton === 'function') options.setStreamButton(false);
        return { routeId: 'open-file-intent', assistantText, routeDecision };
      } catch { /* fall through to file task */ }
    }
    if (routeDecision && routeDecision.route_kind === 'direct_response' && (routeDecision.route === 'light_chat' || routeDecision.route === 'web_search')) {
  ...
  registerMessageRoute({
    id: 'open-file-intent',
    priority: 100,
    match(context: TaskContext) { return matchesOpenFileIntent(context.text); },
  ...
  function buildWhiteboxTaskPayload(text: string, pinnedSelText?: string, pinnedSelSource?: string, overrides?: any): Record<string, any> {
    ...
    const rawFiles: TaskFileInfo[] = Array.isArray(state._aiFileContext)
      ...
      : [];
    const includeCurrentFile = shouldExposeCurrentWorkspaceFile({ text, pinnedSelText } as TaskContext)
      && !(rawFiles.length > 0 && mentionsAttachedFileContext(text));
    const currentFile = includeCurrentFile ? currentWorkspaceTaskFile() : null;
    if (currentFile && mentionsCurrentWorkspaceFile(text) && !rawFiles.some((file) => sameTaskFile(file, currentFile))) rawFiles.unshift(currentFile);
