 ...
 * Restores the comment/revision entry points without loading legacy workspace-assistant.js.
 */

import { $, _escHtml, showToast } from './infrastructure';
...
function _cardHtml(entry: { id: string; item: ReviewItem; kind: string }, index: number): string {
  ...
  return `
    <article class="koto-docx-comment-card${entry.id === _ensureTabReviewState()?.focusedId ? ' is-focused' : ''}${entry.kind === 'proposal' ? ' is-proposal' : ''}" data-review-id="${_escHtml(entry.id)}" tabindex="0">
      <button type="button" class="wa-review-anchor-inline" data-review-action="focus" title="${_escHtml(title)}">${_escHtml(_previewReviewText(anchor, 18) || badge)}</button>
      <div class="koto-docx-comment-card-head">
        <span class="wa-proposal-badge">${badge}</span>
      </div>
      ${isEditing
        ? `<textarea class="koto-docx-comment-edit">${_escHtml(body)}</textarea>
           <div class="koto-docx-comment-actions"><button type="button" data-review-action="save">保存</button><button type="button" data-review-action="delete">删除</button></div>`
        : `<div class="koto-docx-comment-body">${_escHtml(body || '新批注')}</div>
           <div class="koto-docx-comment-actions"><button type="button" data-review-action="edit">编辑</button><button type="button" data-review-action="delete">删除</button></div>`}
    </article>`;
}
...
function _handleReviewShellClick(event: Event): void {
  ...
  const card = actionEl.closest('.koto-docx-comment-card') as HTMLElement | null;
  ...
  if (action === 'focus') {
    ...
    const textarea = card?.querySelector('.koto-docx-comment-edit') as HTMLTextAreaElement | null;
...
(window as any).WA.onDocxCommentsChanged = (comments: any[], tabPath?: string) => {
  ...
  reviewState.comments = (Array.isArray(comments) ? comments : []).map(_normalizeReviewComment);
  _syncDocCommentServerData(targetTab, reviewState);
