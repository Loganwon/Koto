 ...
 * Converted from workspace-assistant.js lines 15634-15766.
 */
...
export function _waQuickActionModelMode(): string {
  ...
  return ['cloud', 'gemini', 'deepseek', 'local'].includes(value) ? value : 'cloud';
}
...
export interface RuntimeConfig {
  ...
  streamWhiteboxTask?: (options: any) => any;
  streamFileTask?: (options: any) => any;
...
export function _initWorkspaceAiRuntimes(): void {
  ...
  if (!_waTaskDispatcher && (window as any).WA && typeof (window as any).WA.createTaskDispatcher === 'function') {
    _waTaskDispatcher = (window as any).WA.createTaskDispatcher({
      state,
      openWorkspaceFile: (path: string) => (window as any).WA.openWorkspaceFile(path),
      ...
      streamTaskFlow: (options: any) => (
        (window as any).WA && typeof (window as any).WA.streamTaskFlow === 'function'
          ? (window as any).WA.streamTaskFlow(options)
          : ((window as any).WA && typeof (window as any).WA.streamWhiteboxTask === 'function'
            ? (window as any).WA.streamWhiteboxTask(options)
            : (window as any).WA.streamFileTask(options))
      ),
      streamFileTask: (options: any) => (window as any).WA.streamFileTask(options),
