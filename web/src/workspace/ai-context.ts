 ...
 * Extracted from workspace-assistant.js lines 1144-1530.
 */
...
export function _renderAIFileChips(): void {
  const targets = [
    ...
    {
      wrap: document.getElementById('wa-session-list-file-chips'),
      list: document.getElementById('wa-session-list-file-chip-list'),
    },
  ...
  targets.forEach(({ wrap, list }) => {
    ...
    if (headerEl) {
      ...
      headerEl.innerHTML =
        ...
        `<button onclick="WA.openNotebookGuide()" title="按当前文档生成摘要、问题和行动项">学习包</button>` +
        `<button onclick="WA.openAudioOverview()" title="按当前文档生成双人讲解脚本">有声概览</button>` +
