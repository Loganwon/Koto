 ...
 *
 * The legacy workspace-assistant.js exposed WA._applyFileJson; the split bundle
 * needs the same contract so fs-tree can apply parsed file responses.
 */
...
function _syncPrimarySaveButtons(tab: TabInfo | null): void {
  ...
  const archiveBtn = document.getElementById('wa-archive-btn') as HTMLButtonElement | null;
  const historyBtn = document.getElementById('wa-history-btn') as HTMLButtonElement | null;
  if (saveBtn) {
    saveBtn.hidden = !tab;
  ...
  if (archiveBtn) archiveBtn.disabled = !tab;
  if (historyBtn) historyBtn.disabled = !tab;
}
...
function _deactivateEditorContainers(): void {
  [
    'wa-docx-editor',
    'wa-xlsx-editor',
    'wa-pptx-editor',
...
function _destroyActiveEditor(): void {
