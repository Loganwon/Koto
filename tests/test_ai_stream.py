...
class TestEditorAIStream:
    ...
    def test_ai_task_action_uses_task_agent(self, app_client):
        ...
        with patch("app.core.agent.task_agent.TaskAgent", FakeTaskAgent), patch.dict(
            "web.app.MODEL_MAP",
            {"CHAT": "gemini-chat-default", "FILE_TASK": "gemini-file-task-default"},
            clear=False,
        ):
            resp = app_client.post(
                "/api/editor/ai/stream",
        ...
        assert any(
            e.get("type") == "phase" and e.get("current") == "analysis" for e in events
        )
    ...
    def test_ai_task_runtime_always_uses_task_agent(self, app_client):
        ...
        events = parse_sse_events(payload)
        assert any(
            e.get("type") == "done" and e.get("summary") == "task agent done"
            for e in events
        )
    ...
    def test_ai_task_loads_persisted_history_when_frontend_omits_history(
        self, app_client
    ):
        captured = {}
        ...
        with patch("app.core.agent.task_agent.TaskAgent", FakeTaskAgent), patch(
            "web.app.session_manager.load", return_value=stored_history
        ):
            resp = app_client.post(
                "/api/editor/ai/stream",
                json={
    ...
    def test_ai_task_persists_runtime_turns_via_session_manager(self, app_client):
        ...
        with patch("app.core.agent.task_agent.TaskAgent", FakeTaskAgent), patch(
            "web.app.session_manager.append_user_early"
        ) as append_user_early, patch(
            "web.app.session_manager.update_last_model_response"
        ) as update_last_model_response:
            resp = app_client.post(
                "/api/editor/ai/stream",
                json={
                    "action": "ai_task",
...
class TestLocalModelMode:
    ...
    def test_workspace_local_toggle_syncs_editor_ai_task_model_keys(self):
        ...
        assert (
            "const editorMode = mode === 'local' ? 'local' : 'cloud';" in toggle_section
        )
        assert (
            "localStorage.setItem('editor_model_mode', editorMode);" in toggle_section
        )
        assert (
    ...
    def test_workspace_cloud_selection_maps_request_model_mode_to_cloud(self):
        ...
        assert (
            "lockedModel: localStorage.getItem('wa_locked_model') === 'local' ? 'local' : 'auto'"
            in js
        )
        assert (
...
class TestAIPanelRegression:
    ...
    def test_check_action_defines_selection_before_use(self):
        ...
        assert pattern.search(
            src
        ), "check branch must derive selection/hasSelection before use"
    ...
    def test_main_ai_stream_payload_includes_file_metadata(self):
        ...
        stream_fetch = send_via_main[
            : send_via_main.find("/api/editor/ai/stream") + 1000
        ]
    ...
    def test_floating_toolbar_routes_standard_actions_via_main_ai(self):
        ...
        src = Path("web/univer-editor/src/FloatingToolbar.js").read_text(
            encoding="utf-8"
        )
        assert (
            "_sendViaMainAI('custom_instruction', this._selectedText, selData, instruction)"
            in src
        )
    ...
    def test_main_ai_uses_shared_progress_consumer_for_phase_and_steps(self):
        ...
        send_via_main = src[src.find("async _sendViaMainAI(") :]
        assert (
            "_consumeTaskProgressEvent(progressState, this._chatFlow, parsed)"
            in send_via_main
        )
    ...
    def test_task_progress_consumer_handles_phase_events(self):
        ...
        src = Path("web/univer-editor/src/AIPanel.js").read_text(encoding="utf-8")
        consume_section = src[
            src.find("_consumeTaskProgressEvent(") : src.find("handleAgentEvent(")
        ]
        assert "case 'phase':" in consume_section
        assert (
            "_computeTaskProgressPercent(phaseIndex, meta.totalPhases"
            in consume_section
        )
    ...
    def test_agent_mode_uses_structured_progress_consumer(self):
        ...
        src = Path("web/univer-editor/src/AIPanel.js").read_text(encoding="utf-8")
        send_via_agent = src[
            src.find("async _sendViaAgent(") : src.find("async _sendViaTask(")
        ]
        assert (
            "_consumeTaskProgressEvent(progressState, this._chatFlow, parsed)"
            in send_via_agent
    ...
    def test_task_mode_uses_structured_progress_consumer(self):
        ...
        src = Path("web/univer-editor/src/AIPanel.js").read_text(encoding="utf-8")
        send_via_task = src[
            src.find("async _sendViaTask(") : src.find("async _sendViaMainAI(")
        ]
        assert (
            "_consumeTaskProgressEvent(progressState, this._chatFlow, ev)"
            in send_via_task
        )
    ...
    def test_skill_exec_handles_plan_tool_and_step_progress(self):
        ...
        src = Path("web/univer-editor/src/AIPanel.js").read_text(encoding="utf-8")
        execute_skill = src[
            src.find("async _executeSkill(") : src.find("_renderSkillOutput(")
        ]
...
class TestSkillExecuteRoute:
    ...
    def test_skill_execute_forwards_structured_progress_events(self, app_client):
        class FakeRuntime:
            def __init__(
                self, socketio=None, model_id="", api_key=None, session_store=None
            ):
                pass
        ...
        with patch(
            "app.core.agent.openclaw_task_runtime.OpenClawTaskRuntime", FakeRuntime
...
class TestTaskExecuteRoute:
    ...
    def test_normalizes_pseudo_model_id_and_defaults_from_file_task_map(
        self, app_client
    ):
        captured = {}
        ...
        with patch(
            ...
            resp = app_client.post(
                ...
                json={
                    "task": "整理当前文件",
                    "files": [
                        {"path": "workspace/a.docx", "name": "a.docx", "type": "docx"}
                    ],
    ...
    def test_skill_execute_defaults_to_file_task_model(self, app_client):
        ...
        with patch(
            ...
            "app.core.skills.skill_manager.SkillManager", FakeSkillManager
        ), patch.dict(
            "web.app.MODEL_MAP",
            {"CHAT": "gemini-chat-default", "FILE_TASK": "gemini-file-task-default"},
            clear=False,
...
class TestEditorEntrypointRegression:
    ...
    def test_editor_entrypoint_no_longer_loads_runtime_patch(self):
        ...
        dist_index = Path("web/static/univer-dist/index.html").read_text(
            encoding="utf-8"
        )
...
class TestTaskAgentDocumentEdits:
    ...
    def test_normalize_file_task_model_id_rejects_unavailable_locked_model(
        self, monkeypatch
    ):
        import web.app as webapp
    ...
    def test_normalize_file_task_model_id_keeps_supported_preview_ids(
        self, monkeypatch
    ):
        import web.app as webapp
...
class TestWorkspaceAssistantOpenClawRegression:
    ...
    def test_workspace_send_message_has_no_dead_multi_file_prompt_branch(self):
        ...
        send_block = src[send_start:send_end]
        assert (
            "state._aiFileContext && state._aiFileContext.length && !_hasAttachedTaskFiles"
            not in send_block
        )
