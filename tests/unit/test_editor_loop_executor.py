...
def test_editor_loop_executor_delegates_text_requests_to_editor_executor(monkeypatch):
    from app.core.agent import editor_loop_executor as executor
    ...
    events = list(executor.EditorLoopExecutor().iter_events(request))

    assert captured["request"] is request
    assert events[0].data["result"] == "rewrite"


def test_editor_loop_executor_delegates_code_mode_to_code_executor(monkeypatch):
    from app.core.agent import editor_loop_executor as executor
