...
class TestCallOllamaChat:
    ...
    def test_returns_error_on_http_failure(self, mocker):
        ...
        mocker.patch(
            "app.core.routing.local_model_router.requests.post", return_value=mock_resp
        )
        content, err = LocalModelRouter.call_ollama_chat(
...
class TestRouterDecision:
    ...
    def test_confidence_str_property(self):
        ...
        d = RouterDecision(
            task_type="CODER", confidence=0.85, latency_ms=42, source="Local"
        )
    ...
    def test_to_legacy_tuple(self):
        ...
        d = RouterDecision(
            task_type="RESEARCH", source="Fallback", confidence=0.5, latency_ms=10
        )
        t = d.to_legacy_tuple()
        assert t[0] == "RESEARCH"
        assert t[2] == "Fallback"
...
class TestRouterLogging:
    def test_no_error_logs_on_offline_classify(self, mocker, caplog):
        ...
        with caplog.at_level(
            logging.ERROR, logger="app.core.routing.local_model_router"
        ):
