...
file_path = (
    r"C:\Users\12524\Desktop\Koto\workspace\documents\王宇轩-简历（美元)_converted.docx"
)
...
print(
    f"Testing with: {os.path.basename(file_path)} ({os.path.getsize(file_path):,} bytes)"
)
...
for line in resp.iter_lines(chunk_size=256, decode_unicode=True):
    ...
    try:
        ...
        if etype == "classification":
            print(
                f'[{elapsed:.0f}s] CLASSIFICATION: task_type={evt.get("task_type")} model={evt.get("model")}'
            )
        elif etype == "progress":
            print(
                f'[{elapsed:.0f}s] PROGRESS {evt.get("progress",0):3d}%: {evt.get("message","")[:80]}'
            )
        elif etype == "info":
            ...
            for sf in saved:
                if os.path.exists(sf):
                    print(
                        f"  ✅ File exists: {os.path.basename(sf)} ({os.path.getsize(sf):,} bytes)"
                    )
                    in_docs = (
                        "workspace\\documents" in sf or "workspace/documents" in sf
                    )
                    print(
                        f'  {"✅ IN workspace/documents" if in_docs else "❌ NOT in workspace/documents"}: {sf}'
                    )
                else:
