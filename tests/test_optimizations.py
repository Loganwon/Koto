...
class TestToolRouter:
    ...
    def router_and_tools(self):
        ...
        tools = [
            {"name": "analyze_excel_data", "description": "分析Excel数据，计算统计指标"},
    ...
    def test_semantic_tier_csv_analysis(self, router_and_tools):
        ...
        names = [t["name"] for t in selected]
        assert "analyze_excel_data" in names, (
    ...
    def test_description_index_cached(self, router_and_tools):
        ...
        assert router._index_cache_key == key_before, "Index should be reused when tool set unchanged"
    ...
    def test_index_rebuilds_on_tool_change(self, router_and_tools):
        ...
        assert router._index_cache_key != key_before, "Index should rebuild when tool set changes"
...
class TestLocalPlannerReplan:
    ...
    def test_replan_signature(self):
        ...
        for required in ["user_input", "completed_steps", "failed_step", "error", "next_id"]:
...
class TestPlanExecutorReplan:
    ...
    async def test_execute_injects_new_steps_on_failure(self):
        ...
        steps = [
            {"id": 1, "task_type": "WEB_SEARCH", "description": "搜索", "output_key": "sr", "depends_on": [], "context_keys": [], "input": "搜索"},
            {"id": 2, "task_type": "FILE_GEN",   "description": "生成", "output_key": "fg", "depends_on": [1], "context_keys": ["sr"], "input": "生成"},
        ]
        ...
        executor = PlanExecutor(steps=steps, user_input="test", handlers=handlers, max_retry=0)

        # Patch _attempt_replan to return a replacement step
        replacement = [
            {"id": 3, "task_type": "FILE_GEN", "description": "替代生成", "output_key": "fg2",
             "depends_on": [], "context_keys": [], "input": "替代生成"}
        ]
...
class TestToolRegistryTimeout:
    ...
    def test_fast_tool_executes_normally(self):
        ...
        reg = ToolRegistry()
        reg.register_tool(
            "double", lambda x: x * 2, "double a number",
    ...
    def test_timeout_raises_runtime_error(self):
        ...
        reg.register_tool("hang", hang, "hangs forever", {"type": "OBJECT", "properties": {}})
        t0 = time.time()
        with pytest.raises(RuntimeError, match="timed out"):
            reg.execute("hang", {})
        elapsed = time.time() - t0
        assert elapsed < _TOOL_TIMEOUT + 5, f"Should timeout around {_TOOL_TIMEOUT}s, took {elapsed:.1f}s"
