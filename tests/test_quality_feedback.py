...
for i, (input_text, expected_contains, ctx) in enumerate(test_cases):
    ...
    if ctx == "ppt_point":
        ...
        if "`" in result and "`" in input_text:
            no_residue = False
        if (
            result.startswith("当然")
            or result.startswith("好的")
...
test_outline = [
    ...
    },
    {
        ...
        "subsections": [
            ...
            {"subtitle": "方案B", "label": "方案B", "points": ["特点1", "特点2"]},
        ],
    },
    {
        "type": "divider",
...
checks = [
    (
        "标题无残留",
        "**" not in sanitized[0]["title"] and "###" not in sanitized[0]["title"],
    ),
    ...
    (
        "对比页AI前缀清除",
        not any("Sure" in p for p in sanitized[1]["subsections"][0]["points"]),
    ),
        ...
        "subsections": [
            {
                "subtitle": "医疗",
                "label": "医疗",
                "points": ["AI辅助诊断准确率达95%以上", "药物研发周期缩短40%"],
            },
            {
                "subtitle": "金融",
                "label": "金融",
...
result_good = FileQualityEvaluator.evaluate_ppt_outline(
    good_outline, "做一个关于人工智能的PPT"
)
print(f"  好大纲评分: {result_good['score']}/100, pass={result_good['pass']}")
print(f"  类型: {result_good['metrics']['types_used']}")
print(
    f"  总要点: {result_good['metrics']['total_points']}, 平均: {result_good['metrics']['avg_points']}"
)
...
qg_result = FileQualityGate.check_and_fix_ppt_outline(
    contaminated_outline, "做一个关于AI的PPT", _test_progress
)
...
doc_result = FileQualityGate.check_and_fix_document(
    doc_text, "写一篇AI报告", _test_doc_progress
)
...
fb.ppt_outline_ready(
    10, title="人工智能发展趋势", type_summary="详细×6, 概览×2, 亮点×1, 过渡×1"
)
fb.ppt_enriching(3)
fb.ppt_enriched(3)
fb.ppt_images(4)
...
try:
    ...
    # Step 1: 质量门控（清洗+评估）
    qg = FileQualityGate.check_and_fix_ppt_outline(
        test_outline, "做一个AI市场分析的PPT"
    )
